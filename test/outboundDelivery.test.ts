...
import { acceptOutboundMessage, claimOutboundMessage, claimOutboundMessageV2, releaseOutboundMessage } from "../src/outboundDelivery";
...
describe("claimOutboundMessageV2", () => {
  it("calls the RPC with the documented URL, method, headers, and an empty body", async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse([claimedRowV2]));
    vi.stubGlobal("fetch", fetchMock);

    const result = await claimOutboundMessageV2(env);

    expect(result).toEqual(claimedParsedV2);
    expect(fetchMock).toHaveBeenCalledTimes(1);
    const [url, init] = fetchMock.mock.calls[0] as [URL, RequestInit];
    expect(url.toString()).toBe("https://example.supabase.co/rest/v1/rpc/claim_outbound_message_v2");
    expect(init.method).toBe("POST");
    const headers = init.headers as Record<string, string>;
    expect(headers.apikey).toBe("test-service-role-key");
    expect(headers.authorization).toBe("Bearer test-service-role-key");
  ...
  it.each(["exhausted", "empty"] as const)("parses a %s result with all other fields null", async (result) => {
    ...
    expect(await claimOutboundMessageV2(env)).toEqual({ kind: result });
  });

  it("fails closed when Supabase configuration is missing, without calling fetch", async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const result = await claimOutboundMessageV2({ ...env, SUPABASE_URL: "" });
    expect(result).toEqual({ kind: "failed" });
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it("fails closed for plain HTTP against a non-loopback host", async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const result = await claimOutboundMessageV2({ ...env, SUPABASE_URL: "http://example.supabase.co" });
    expect(result).toEqual({ kind: "failed" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("allows plain HTTP for loopback localhost testing", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(jsonResponse([claimedRowV2])));
    const result = await claimOutboundMessageV2({ ...env, SUPABASE_URL: "http://localhost:54321" });
    expect(result).toEqual(claimedParsedV2);
  });

  it("treats a network failure as failed", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("network down")));
    expect(await claimOutboundMessageV2(env)).toEqual({ kind: "failed" });
  });
  ...
  it.each([
    ["a non-array body", claimedRowV2],
    ["zero rows", []],
    ["more than one row", [claimedRowV2, claimedRowV2]],
    ["a row with an extra column", [{ ...claimedRowV2, extra: "x" }]],
    ["a row with an unknown result", [{ ...claimedRowV2, result: "invented" }]],
    ["a claimed row with a non-string outbox_id", [{ ...claimedRowV2, outbox_id: 1 }]],
    ["a claimed row with a malformed outbox_id", [{ ...claimedRowV2, outbox_id: "not-a-uuid" }]],
    ["a claimed row with a non-string claim_token", [{ ...claimedRowV2, claim_token: 1 }]],
    ["a claimed row with a malformed claim_token", [{ ...claimedRowV2, claim_token: "not-a-uuid" }]],
    ["a claimed row with a non-string whatsapp_account_id", [{ ...claimedRowV2, whatsapp_account_id: 1 }]],
    ["a claimed row with a malformed whatsapp_account_id", [{ ...claimedRowV2, whatsapp_account_id: "not-a-uuid" }]],
    ["a claimed row with an empty phone_number_id", [{ ...claimedRowV2, phone_number_id: "" }]],
    ["a claimed row with a non-numeric phone_number_id", [{ ...claimedRowV2, phone_number_id: "abc123" }]],
    ["a claimed row with an empty recipient_e164", [{ ...claimedRowV2, recipient_e164: "" }]],
    ["a claimed row with a recipient_e164 missing the leading +", [{ ...claimedRowV2, recipient_e164: "15550011111" }]],
    ["a claimed row with a recipient_e164 leading zero", [{ ...claimedRowV2, recipient_e164: "+05550011111" }]],
    ["a claimed row with empty content", [{ ...claimedRowV2, content: "" }]],
    ["a claimed row with oversized content", [{ ...claimedRowV2, content: "a".repeat(4097) }]],
    ["a claimed row with a zero attempt_count", [{ ...claimedRowV2, attempt_count: 0 }]],
    ["a claimed row with an attempt_count above three", [{ ...claimedRowV2, attempt_count: 4 }]],
    ["a claimed row with a non-integer attempt_count", [{ ...claimedRowV2, attempt_count: 1.5 }]],
    [
      "an exhausted row with a non-null outbox_id",
      [
        {
          result: "exhausted",
          outbox_id: outboxId,
    ...
    [
      "an empty row with a non-null content",
      [
        {
          result: "empty",
          outbox_id: null,
  ...
  it("rejects a non-plain row and hidden extra columns", async () => {
    const nonPlain = Object.assign(Object.create(null), claimedRowV2);
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(rawJsonResponse([nonPlain])));
    expect(await claimOutboundMessageV2(env)).toEqual({ kind: "failed" });

    const hiddenExtra = { ...claimedRowV2 };
    Object.defineProperty(hiddenExtra, "extra", { value: "x" });
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(rawJsonResponse([hiddenExtra])));
    expect(await claimOutboundMessageV2(env)).toEqual({ kind: "failed" });
  });
  ...
  it("does not log the request or response", async () => {
    ...
    await claimOutboundMessageV2(env);
