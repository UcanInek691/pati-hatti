...
describe("sendWhatsAppTextMessage", () => {
  it("calls the documented URL, method, headers, and exact request body", async () => {
    ...
    const result = await sendWhatsAppTextMessage(phoneNumberId, recipientE164, content, env);
    ...
    expect(headers.authorization).toBe("Bearer test-whatsapp-access-token");
  ...
  it.each([
    ...
    await sendWhatsAppTextMessage(phoneNumberId, recipientE164, fixedText, env);
  ...
  it("tolerates unrelated top-level Meta response fields", async () => {
    ...
    expect(await sendWhatsAppTextMessage(phoneNumberId, recipientE164, content, env)).toEqual({
      kind: "accepted",
  ...
  it.each([
    ["a blank token", { ...env, WHATSAPP_ACCESS_TOKEN: "   " }],
    ["a malformed graph version", { ...env, WHATSAPP_GRAPH_API_VERSION: "25.0" }],
    ["a graph version with a non-zero minor", { ...env, WHATSAPP_GRAPH_API_VERSION: "v25.1" }],
  ])("fails closed without calling fetch for %s", async (_label, badEnv) => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    expect(await sendWhatsAppTextMessage(phoneNumberId, recipientE164, content, badEnv)).toEqual({ kind: "failed" });
  ...
  it.each([
    ...
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    expect(await sendWhatsAppTextMessage(badPhoneNumberId, badRecipient, badContent, env)).toEqual({ kind: "failed" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("treats a network failure as failed", async () => {
    vi.stubGlobal("fetch", vi.fn().mockRejectedValue(new Error("network down")));
    expect(await sendWhatsAppTextMessage(phoneNumberId, recipientE164, content, env)).toEqual({ kind: "failed" });
  });
  ...
  it("does not log the request or response", async () => {
    ...
    await sendWhatsAppTextMessage(phoneNumberId, recipientE164, content, env);
