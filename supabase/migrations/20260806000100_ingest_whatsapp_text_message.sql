-- Atomic inbound WhatsApp text message ingestion for VetAI.
-- Validated in the disposable `vetai-test` project with
-- supabase/tests/005_ingest_whatsapp_text_message.sql. Production must apply
-- this file through the managed Supabase migration workflow.
...
-- verified need.
create unique index conversations_one_open_per_owner_idx
  on public.conversations (clinic_id, owner_id)
