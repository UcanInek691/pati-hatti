...
| Klinik e-posta uyarı tercihleri ve rollout anahtarı (Task 056; direct-query migration + rollback fixture yalnız disposable `vetai-test` üzerinde geçti, migration history değişmedi; staging/production/gerçek e-posta aktivasyonu **NOT RUN** — bkz. [`operational-alerting.md`](operational-alerting.md) §11) | `clinic_alert_settings`: klinik başına platform-admin rollout açık/kapalı bayrağı; `clinic_alert_gate_audit`: bu bayrağın her gerçek geçişinin aktör Auth kullanıcı kimliği ve zaman damgasıyla kaydı; `clinic_alert_recipients.enabled_at`: kişisel tercihin DB-zamanlı etkinleşme epoch'u | Bir personelin `clinic_alert_recipients` üzerindeki mevcut kişisel aboneliğinin, ilgili klinik için ayrıca platform admin tarafından açılmış olup olmadığını belirleme — Task 053'teki teslim mekanizmasını değiştirmez, yalnız kapsamını daraltır | Supabase; her iki tablo da RLS açık, politika yok; `clinic_alert_settings` `service_role`'a yazılabilir, `clinic_alert_gate_audit`'e **hiçbir role** (service_role dahil) doğrudan yetki verilmez, yalnız `SECURITY DEFINER` RPC üzerinden eklenir. Personel kendi tercihini yalnız kendi `auth.uid()`'i için değiştirebilir; e-posta adresi istemciden hiç alınmaz, yalnız çağıranın kendi onaylı Auth e-postası kullanılır |
