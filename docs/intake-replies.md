...
It is not wired into the Queue consumer (`src/intakeConsumer.ts`) or any
runtime path. Nothing is persisted, queued, or sent to WhatsApp as a result of
...
No reply produced here is persisted, queued, generated by an LLM, or sent to
WhatsApp. Direct sending from the Queue consumer would create a
lost-message or duplicate-message window, so runtime wiring stays out of
scope until an atomic outbox boundary exists that ties message send to
conversation-state finalization.
