...
export type ClaimOutboundMessageV2Result =
  | {
      kind: "claimed";
      outboxId: string;
      claimToken: string;
...
const FAILED_CLAIM_V2: ClaimOutboundMessageV2Result = { kind: "failed" };
...
/**
 * Calls the `claim_outbound_message_v2` Data API RPC over native fetch. Never
 * logs the request/response body. Reserved for new Worker code; the old
 * `claim_outbound_message` RPC remains solely as a rollback target.
 */
export async function claimOutboundMessageV2(env: Env): Promise<ClaimOutboundMessageV2Result> {
  const endpoint = buildEndpoint(env, "claim_outbound_message_v2");
  if (!endpoint) return FAILED_CLAIM_V2;

  const rows = await callRpc(endpoint, env, {});
  if (rows === null || rows.length !== 1) return FAILED_CLAIM_V2;

  const row = asPlainRecord(rows[0]);
  try {
    if (!row || Reflect.ownKeys(row).length !== 8) return FAILED_CLAIM_V2;
    ...
    if (result !== "claimed") return FAILED_CLAIM_V2;
    if (typeof outboxId !== "string" || !UUID_PATTERN.test(outboxId)) return FAILED_CLAIM_V2;
    if (typeof claimToken !== "string" || !UUID_PATTERN.test(claimToken)) return FAILED_CLAIM_V2;
    if (typeof whatsappAccountId !== "string" || !UUID_PATTERN.test(whatsappAccountId)) return FAILED_CLAIM_V2;
    if (typeof phoneNumberId !== "string" || !PHONE_NUMBER_ID_PATTERN.test(phoneNumberId)) return FAILED_CLAIM_V2;
    if (typeof recipientE164 !== "string" || !E164_PATTERN.test(recipientE164)) return FAILED_CLAIM_V2;
    if (typeof content !== "string" || !isCodePointLengthInRange(content, 1, 4096)) return FAILED_CLAIM_V2;
    if (typeof attemptCount !== "number" || !Number.isInteger(attemptCount) || attemptCount < 1 || attemptCount > 3) return FAILED_CLAIM_V2;

    return { kind: "claimed", outboxId, claimToken, whatsappAccountId, phoneNumberId, recipientE164, content, attemptCount };
  } catch {
    return FAILED_CLAIM_V2;
  }
}
