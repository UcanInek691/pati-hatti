...
import { acceptOutboundMessage, claimOutboundMessage, releaseOutboundMessage } from "./outboundDelivery";
...
function hasValidMetaConfig(env: Env): boolean {
  return env.WHATSAPP_ACCESS_TOKEN.trim().length > 0 && GRAPH_VERSION_PATTERN.test(env.WHATSAPP_GRAPH_API_VERSION);
}

/**
 * Drains at most `MAX_OUTBOUND_ROWS_PER_RUN` pending outbox rows: claim, send
 * once through Meta, then accept or release exactly once per row. At-least-once
 * delivery only — a lost acceptance response can produce a rare duplicate send
 * on a later run. Never throws; never logs identifiers, recipients, content,
 * or secrets.
 */
export async function drainOutboundMessages(env: Env): Promise<void> {
  ...
  for (let i = 0; i < MAX_OUTBOUND_ROWS_PER_RUN; i++) {
    const claim = await claimOutboundMessage(env);

    if (claim.kind === "empty" || claim.kind === "failed") return;
    if (claim.kind === "exhausted") continue;

    const send = await sendWhatsAppTextMessage(claim.phoneNumberId, claim.recipientE164, claim.content, env);
